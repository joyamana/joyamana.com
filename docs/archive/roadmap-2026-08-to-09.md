# Archived Project Roadmap — 2026-08 to 2026-09

Status: Historical phase record  
Archived: 2026-09-02

本文件保存 Phase 0–4 的原始阶段结构、日期化实施结果和当时的退出条件。当前优先级只看
`../ROADMAP.md`；本文件不得作为当前 blocker、测试数量或部署状态来源。
...
Analytics/consent 和完整发布验收尚未完成。下单支付已完整支持，Payment test mode
流程测试未发现问题；Production 的 en-US/es-US Core、Commerce、Policies 已开放索引。
...
代码已完成；履约模式和法律/审批输入已解决，仍因特殊地址覆盖、履约费率、税费/进口
责任等剩余运营配置仍需按范围完成。仓库示例值和未配置时默认值继续保持跳转门禁关闭，
但当前 Production Checkout 已经业务方验收并启用；各环境值仍须单独验收。
...
Production 当前已打开索引总门禁；en-US/es-US Core、Commerce、Policies 可索引，
Editorial 继续关闭，sitemap 已非空。D-044 已确认 `www` canonical 与 apex 308；当前
Production 的 canonical 与 `og:url` 已输出 `www` origin。
...
- `info@joyamana.com` 已确认可以收信，当前正式支持方式为 Email-only；Contact 表单、
  Resend 与滥用防护延后；负责人/备援、外发认证、回复流程与垃圾箱表现已确认。
