# Archived Open Questions and Audit Snapshots — 2026-08 to 2026-09

Status: Historical evidence only  
Archived: 2026-09-02

本文件保留早期开放问题、Shopify/Production 审计快照和已解决记录。它不是当前 blocker
清单；当前状态只使用 `../OPEN_QUESTIONS.md`，并以 `../DECISIONS.md` 的 Accepted
决策为准。

## Original current-state document
...
剩余问题只阻塞其 `Blocks` 所列的配送覆盖、Checkout 设置或税费/政策能力，不再作为
全站、Catalog 或已审核内容的笼统 blocker。缺失的 Shopify Policy 仍必须 fail closed
或明确显示暂不可用，不得输出与公开正文不一致的 Schema，也不得用本地开发正文伪装政策。
...
- [x] Logo、字体授权、颜色已由业务方确认完成（2026-09-02）
- [x] 真实商品摄影/视频已由业务方确认完成（2026-09-02）
...
- [x] `info@joyamana.com` 负责人/备援、外发认证与回复/垃圾箱表现已确认
  （2026-09-02）
...
## Resolved / removed from website scope on 2026-09-02

- Q-001A、Q-001B、Q-002A、Q-002B、Q-002C 已由业务方关闭。这些商标/账号、商业
  规划、具体商品开发、礼赠运营和 assortment 决定在网站项目外管理，不再作为代码、
  Catalog、索引或 Checkout 的仓库门禁。网站继续只读取 Shopify 已发布的真实价格、
  商品和库存，不在仓库复制 margin、内部审批清单或未发布商品事实。
- Q-003A、Q-003F 已由业务方确认解决，不再阻塞 Shipping、Privacy、Terms 或
  Organization 的网站实施。具体履约安排、法律主体、地址和审批记录不在本文件重复；
  客户可见事实以已批准并发布的 Shopify 配置/Policy 为准。
- Organization/Site Settings 的规范化实体和相应 Schema 仍是代码实现缺口；它不再是
  等待 Q-003F 决策的业务输入缺口，实施时只能映射已批准的公开字段。
- `info@joyamana.com` 的负责人/备援、外发认证和投递表现，以及 Logo、字体授权、颜色、
  真实商品摄影/视频均已由业务方确认完成，不再列为发布待办。

## Production deployment follow-up — 2026-08-31（历史快照）
...
## Production indexing and Checkout follow-up — 2026-09-02

- 仓库 `src/config/indexing.ts` 当前对 en-US/es-US 的 Core、Commerce、Policies 均为
  `true`，Editorial 为 `false`。Production 部署总开关已实际生效：公开首页与
  `/es-us` 均返回 `index, follow`，canonical 使用 `https://www.joyamana.com`。
- 公开 `sitemap.xml` 已非空，包含 en-US/es-US 的已就绪 Core、Commerce、Policies
  URL；不包含 `/blog` 或 `/crystals`。公开 `/blog` 仍为
  `noindex, nofollow, noarchive`，与 Editorial scope 关闭一致。
- `/cart`、Search、Preview、未知路径和参数请求继续永久 noindex；公开
  `/shop?sort=price` 已复核为 `noindex, nofollow, noarchive`，并 canonical 回
  `/shop`。单页内容 readiness 仍可在 scope 开启时排除缺失或 fallback 页面。
- 以上 2026-09-02 公开响应取代 2026-08-31“Production 全站 noindex、空 sitemap”的
  当前状态描述；旧段落只保留为历史部署证据。
- 业务方确认下单与支付已完整支持，Shopify Payment test mode 下的下单、支付与后续
  流程测试未发现问题。Checkout 不再作为未实现功能或开放问题；test mode 结果不应被
  改写为未执行的 live charge/payout 对账证据。
