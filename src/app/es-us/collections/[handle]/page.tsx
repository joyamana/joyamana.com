import type { Metadata } from "next";
import { CollectionPage } from "@/components/pages/collection-page";
import { getDesignCollection } from "@/lib/commerce/catalog";
import {
  buildMetadata,
  buildNoIndexMetadata,
  getCollectionSeoDescription,
  type PageSearchParams,
} from "@/lib/seo";

export const dynamic = "force-dynamic";

export async function generateMetadata({
  params,
  searchParams,
}: {
  params: Promise<{ handle: string }>;
  searchParams: Promise<PageSearchParams>;
}): Promise<Metadata> {
  const { handle } = await params;
  const collection = await getDesignCollection(handle, "us", "es-US");
  const description = getCollectionSeoDescription(collection);
  if (!collection || !description) {
    return buildNoIndexMetadata({
      title: collection?.title || "Colección no disponible",
      description: collection
        ? "Explora las piezas de esta colección de diseño de Joya Mana."
        : "Esta colección no está disponible actualmente.",
    });
  }
  return buildMetadata({
    title: collection.seoTitle || collection.title,
    description,
    locale: "es-US",
    path: `/collections/${handle}`,
    searchParams: await searchParams,
  });
}

export default async function Page({
  params,
  searchParams,
}: {
  params: Promise<{ handle: string }>;
  searchParams: Promise<PageSearchParams>;
}) {
  return <CollectionPage locale="es-US" handle={(await params).handle} searchParams={await searchParams} />;
}
